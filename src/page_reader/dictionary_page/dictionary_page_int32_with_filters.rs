...
struct DictionaryValueInt32 {
    pub selectivity: bool,
    pub value: i32,
}

impl DictionaryValueInt32 {
    #[inline(always)]
    pub fn new(selectivity: bool, value: i32) -> DictionaryValueInt32 {
        DictionaryValueInt32 { selectivity, value }
    }
}

impl Display for DictionaryValueInt32 {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        write!(f, "({}, {})", self.value, self.selectivity)
    }
}

pub struct DictionaryPageWithFilterInt32 {
    num_values: usize,
    sorted: bool,
    type_size: usize,
    #[allow(dead_code)]
    zero_copy: bool,
    dictionary: Vec<DictionaryValueInt32>,
}

#[allow(dead_code)]
impl std::fmt::Display for DictionaryPageWithFilterInt32 {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
...
impl DictionaryPageNew for DictionaryPageWithFilterInt32 {
    #[inline(always)]
...
impl DictionaryPageWithFilterInt32 {
    pub fn new(
        page_header: &PageHeader,
        buffer: &mut dyn ByteBufferBase,
        type_size: usize,
        filter: &dyn FixedLengthRangeFilter,
    ) -> Result<DictionaryPageWithFilterInt32, BoltReaderError> {
        ...
        let mut dictionary: Vec<DictionaryValueInt32> = Vec::with_capacity(num_values);

        for data in dictionary_data.iter().take(num_values) {
            dictionary.push(DictionaryValueInt32::new(filter.check_i32(*data), *data));
        }
        ...
        Ok(DictionaryPageWithFilterInt32 {
            num_values,
...
mod tests {
    ...
    use crate::page_reader::dictionary_page::dictionary_page_base::DictionaryPageNew;
    use crate::page_reader::dictionary_page::dictionary_page_int32_with_filters::{
        DictionaryPageWithFilterInt32, DictionaryValueInt32,
    };
    use crate::utils::byte_buffer_base::ByteBufferBase;
    use crate::utils::direct_byte_buffer::{Buffer, DirectByteBuffer};
    use crate::utils::exceptions::BoltReaderError;
    ...
    fn load_dictionary_page_int32_with_filter<'a>(
        ...
        DictionaryPageWithFilterInt32::new(&page_header, buf, 4, filter)
    }

    fn verify_dictionary_result(
        dictionary: &[DictionaryValueInt32],
        filter: &dyn FixedLengthRangeFilter,
    ) {
        for elem in dictionary {
            assert_eq!(filter.check_i32(elem.value), elem.selectivity);
        }
    }
    ...
    fn test_create_and_drop_dictionary_page() {
        ...
        let res = load_dictionary_page_int32_with_filter(&mut buf, &filter);
