...
struct DictionaryValueInt64 {
    pub selectivity: bool,
    pub value: i64,
}

impl DictionaryValueInt64 {
    #[inline(always)]
    pub fn new(selectivity: bool, value: i64) -> DictionaryValueInt64 {
        DictionaryValueInt64 { selectivity, value }
    }
}

impl Display for DictionaryValueInt64 {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        write!(f, "({}, {})", self.value, self.selectivity)
    }
}

pub struct DictionaryPageWithFilterInt64 {
    num_values: usize,
    sorted: bool,
    type_size: usize,
    #[allow(dead_code)]
    zero_copy: bool,
    dictionary: Vec<DictionaryValueInt64>,
}

#[allow(dead_code)]
impl std::fmt::Display for DictionaryPageWithFilterInt64 {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
...
impl DictionaryPageNew for DictionaryPageWithFilterInt64 {
    #[inline(always)]
...
impl DictionaryPageWithFilterInt64 {
    pub fn new(
        page_header: &PageHeader,
        buffer: &mut dyn ByteBufferBase,
        type_size: usize,
        filter: &dyn FixedLengthRangeFilter,
    ) -> Result<DictionaryPageWithFilterInt64, BoltReaderError> {
        ...
        let mut dictionary: Vec<DictionaryValueInt64> = Vec::with_capacity(num_values);

        for data in dictionary_data.iter().take(num_values) {
            dictionary.push(DictionaryValueInt64::new(filter.check_i64(*data), *data));
        }
        ...
        Ok(DictionaryPageWithFilterInt64 {
            num_values,
...
mod tests {
    ...
    use crate::page_reader::dictionary_page::dictionary_page_base::DictionaryPageNew;
    use crate::page_reader::dictionary_page::dictionary_page_int64_with_filters::{
        DictionaryPageWithFilterInt64, DictionaryValueInt64,
    };
    use crate::utils::byte_buffer_base::ByteBufferBase;
    use crate::utils::direct_byte_buffer::{Buffer, DirectByteBuffer};
    use crate::utils::exceptions::BoltReaderError;
    ...
    fn load_dictionary_page_int64_with_filter<'a>(
        ...
        DictionaryPageWithFilterInt64::new(&page_header, buf, 8, filter)
    }

    fn verify_dictionary_result(
        dictionary: &[DictionaryValueInt64],
        filter: &dyn FixedLengthRangeFilter,
    ) {
        for elem in dictionary {
            assert_eq!(filter.check_i64(elem.value), elem.selectivity);
        }
    }
