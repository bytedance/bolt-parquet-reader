...
pub struct PlainDataPageReaderFloat32V1<'a> {
    ...
    data: Vec<f32>,
    data_with_nulls: Option<Vec<f32>>,
}
...
impl<'a> DataPage<f32> for PlainDataPageReaderFloat32V1<'a> {
    ...
    fn read(
        ...
        result_bridge: &mut dyn Bridge<f32>,
...
impl<'a> PlainDataPageReaderFloat32V1<'a> {
    pub fn new(
        ...
        let data_with_nulls = if has_null {
            let mut vec: Vec<f32> = Vec::with_capacity(num_values);
    ...
    pub fn read_nullable_with_filter(
        ...
        for i in start..end {
            let filter_res = filter.check_f32_with_validity(data_with_nulls[i], validity[i]);
            generator.update(i + self.current_offset - offset, filter_res);
            if filter_res {
