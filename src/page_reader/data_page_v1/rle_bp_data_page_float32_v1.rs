...
pub struct RleBpDataPageReaderFloat32V1<'a> {
    has_null: bool,
    num_values: usize,
...
#[allow(dead_code)]
impl<'a> std::fmt::Display for RleBpDataPageReaderFloat32V1<'a> {
...
impl<'a> DataPageNew for RleBpDataPageReaderFloat32V1<'a> {
    ...
    fn read(
        ...
        let finished = if self.has_null {
                ...
                if validity[i] {
                    data_with_nulls[i] = Some(
                        self.dictionary_page
                            .find_float32(self.data[self.non_null_index] as usize),
                    );
            ...
            for i in start..end {
                result_bridge.append_non_null_float32_result(
                    self.dictionary_page.find_float32(self.data[i] as usize),
                )?;
            }
            end == self.num_values
        };
...
impl<'a> RleBpDataPageReaderFloat32V1<'a> {
    pub fn new(
        ...
        validity: Option<Vec<bool>>,
        dictionary_page: Rc<DictionaryPageEnum>,
    ) -> Result<RleBpDataPageReaderFloat32V1<'a>, BoltReaderError> {
        ...
        Ok(RleBpDataPageReaderFloat32V1 {
            has_null,
            num_values,
            current_offset,
            type_size,
    ...
    pub fn read_non_null_with_filter(
        ...
        for i in start..end {
            ...
            if filter_res {
                result_bridge
                    .append_non_null_float32_result(self.dictionary_page.find_float32(index))?;
            }
        }
    ...
    pub fn read_nullable_with_filter(
        ...
        for i in self.nullable_index..end {
            if validity[i] {
                ...
                data_with_nulls[i] = Some(self.dictionary_page.find_float32(index));
...
mod tests {
    ...
    fn load_dictionary_page(buf: &mut dyn ByteBufferBase) -> Rc<DictionaryPageEnum> {
        ...
        let dictionary_page = DictionaryPageFloat32::new(&page_header, buf, mem::size_of::<f32>());
        assert!(dictionary_page.is_ok());
        Rc::from(DictionaryPageEnum::DictionaryPageFloat32(
    ...
    fn load_dictionary_page_with_filter(
        ...
        let page_header = page_header.unwrap();
        let dictionary_page =
            DictionaryPageWithFilterFloat32::new(&page_header, buf, mem::size_of::<f32>(), filter);
        assert!(dictionary_page.is_ok());
        Rc::from(DictionaryPageEnum::DictionaryPageWithFilterFloat32(
            dictionary_page.unwrap(),
        ))
    }
    ...
    fn load_rle_bp_page<'a>(
        ...
        let data_page = RleBpDataPageReaderFloat32V1::new(
            &page_header,
            buf,
            offset,
            4,
