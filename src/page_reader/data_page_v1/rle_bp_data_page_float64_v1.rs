...
pub struct RleBpDataPageReaderFloat64V1<'a> {
    has_null: bool,
    num_values: usize,
...
#[allow(dead_code)]
impl<'a> std::fmt::Display for RleBpDataPageReaderFloat64V1<'a> {
...
impl<'a> DataPage for RleBpDataPageReaderFloat64V1<'a> {
    ...
    fn read(
        ...
        let finished = if self.has_null {
                ...
                if validity[i] {
                    data_with_nulls[i] = Some(
                        self.dictionary_page
                            .find_float64(self.data[self.non_null_index] as usize),
                    );
            ...
            for i in start..end {
                result_bridge.append_non_null_float64_result(
                    self.dictionary_page.find_float64(self.data[i] as usize),
                )?;
            }
            end == self.num_values
        };
...
impl<'a> RleBpDataPageReaderFloat64V1<'a> {
    pub fn new(
        ...
        Ok(RleBpDataPageReaderFloat64V1 {
            has_null,
            num_values,
            current_offset,
            type_size,
    ...
    pub fn read_non_null_with_filter(
        ...
        for i in start..end {
            ...
            if filter_res {
                result_bridge
                    .append_non_null_float64_result(self.dictionary_page.find_float64(index))?;
            }
        }
    ...
    pub fn read_nullable_with_filter(
        ...
        for i in self.nullable_index..end {
            if validity[i] {
                ...
                data_with_nulls[i] = Some(self.dictionary_page.find_float64(index));
                nullable_selectivity[i] = self.dictionary_page.validate(index);

                self.non_null_index += 1;
            } else {
                nullable_selectivity[i] =
                    filter.check_f64_with_validity(f64::default(), validity[i]);
            }
        }
...
mod tests {
    ...
    fn load_dictionary_page(buf: &mut dyn ByteBufferBase) -> Rc<DictionaryPageEnum> {
        ...
        let dictionary_page = DictionaryPageFloat64::new(
            &page_header,
            buf,
            mem::size_of::<f64>(),
            true,
    ...
    fn load_dictionary_page_with_filter(
        ...
        assert!(page_header.is_ok());
        let page_header = page_header.unwrap();
        let dictionary_page = DictionaryPageWithFilterFloat64::new(
            &page_header,
            buf,
            mem::size_of::<f64>(),
            true,
    ...
    fn load_rle_bp_page<'a>(
        ...
        let data_page = RleBpDataPageReaderFloat64V1::new(
            &page_header,
            buf,
            offset,
            8,
    ...
    fn test_read_non_null_data_page() {
        ...
        while begin < num_values {
            ...
            assert!(res.is_ok());
            verify_rle_bp_float64_non_null_result(&result_row_range_set, &raw_bridge, None);
            begin = end;
        }
    }
