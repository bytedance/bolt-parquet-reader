...
pub struct PlainDataPageReaderFloat64V1<'a> {
    ...
    data: Vec<f64>,
    data_with_nulls: Option<Vec<f64>>,
}
...
impl<'a> DataPage<f64> for PlainDataPageReaderFloat64V1<'a> {
    ...
    fn read(
        ...
        result_bridge: &mut dyn Bridge<f64>,
...
impl<'a> PlainDataPageReaderFloat64V1<'a> {
    pub fn new(
        ...
        let data_with_nulls = if has_null {
            let mut vec: Vec<f64> = Vec::with_capacity(num_values);
    ...
    pub fn read_nullable_with_filter(
        ...
        for i in start..end {
            let filter_res = filter.check_f64_with_validity(data_with_nulls[i], validity[i]);
            generator.update(i + self.current_offset - offset, filter_res);
            if filter_res {
