...
pub struct RleBpDataPageReaderByteArrayV1<'a> {
    has_null: bool,
    num_values: usize,
...
#[allow(dead_code)]
impl<'a> std::fmt::Display for RleBpDataPageReaderByteArrayV1<'a> {
...
impl<'a> DataPage for RleBpDataPageReaderByteArrayV1<'a> {
    ...
    fn read(
        ...
        let finished = if self.has_null {
                ...
                if validity[i] {
                    data_with_nulls[i] = Some(
                        self.dictionary_page
                            .find_byte_array(self.data[self.non_null_index] as usize),
                    );
            ...
            for i in start..end {
                result_bridge.append_non_null_byte_array_result(
                    self.dictionary_page.find_byte_array(self.data[i] as usize),
                )?;
            }
            end == self.num_values
        };
...
impl<'a> RleBpDataPageReaderByteArrayV1<'a> {
    pub fn new(
        ...
        validity: Option<Vec<bool>>,
        dictionary_page: Rc<DictionaryPageEnum>,
    ) -> Result<RleBpDataPageReaderByteArrayV1<'a>, BoltReaderError> {
        ...
        Ok(RleBpDataPageReaderByteArrayV1 {
            has_null,
            num_values,
            current_offset,
            type_size,
    ...
    pub fn read_non_null_with_filter(
        ...
        for i in start..end {
            ...
            if filter_res {
                result_bridge.append_non_null_byte_array_result(
                    self.dictionary_page.find_byte_array(index),
                )?;
            }
        }
    ...
    pub fn read_nullable_with_filter(
        ...
        for i in self.nullable_index..end {
            if validity[i] {
                ...
                data_with_nulls[i] = Some(self.dictionary_page.find_byte_array(index));
        ...
        for i in start..end {
            ...
            if nullable_selectivity[i] {
                result_bridge.append_nullable_byte_array_result(data_with_nulls[i].clone())?;
            }
        }
        generator.finish(end + self.current_offset - offset);
...
mod tests {
    ...
    fn load_dictionary_page(buf: &mut dyn ByteBufferBase) -> Rc<DictionaryPageEnum> {
        ...
        let dictionary_page = DictionaryPageByteArray::new(
            &page_header,
            buf,
            1,
            true,
        ...
        Rc::from(DictionaryPageEnum::DictionaryPageByteArray(
    ...
    fn load_rle_bp_page<'a>(
        ...
        let data_page = RleBpDataPageReaderByteArrayV1::new(
            &page_header,
            buf,
            offset,
            1,
    ...
    fn test_read_non_null_data_page() {
        ...
        while begin < num_values {
            ...
            assert!(res.is_ok());
            verify_rle_bp_byte_array_non_null_result(&result_row_range_set, &raw_bridge);
            begin = end;
        }
    }
