...
pub struct RleBpDataPageReaderInt32V1<'a> {
    has_null: bool,
    num_values: usize,
...
#[allow(dead_code)]
impl<'a> std::fmt::Display for RleBpDataPageReaderInt32V1<'a> {
...
impl<'a> DataPageNew for RleBpDataPageReaderInt32V1<'a> {
    ...
    fn read(
        ...
        let finished = if self.has_null {
                ...
                if validity[i] {
                    data_with_nulls[i] = Some(
                        self.dictionary_page
                            .find_int32(self.data[self.non_null_index] as usize),
                    );
            ...
            for i in start..end {
                result_bridge.append_non_null_int32_result(
                    self.dictionary_page.find_int32(self.data[i] as usize),
                )?;
            }
            end == self.num_values
        };
...
impl<'a> RleBpDataPageReaderInt32V1<'a> {
    pub fn new(
        ...
        validity: Option<Vec<bool>>,
        dictionary_page: Rc<DictionaryPageEnum>,
    ) -> Result<RleBpDataPageReaderInt32V1<'a>, BoltReaderError> {
    ...
    pub fn read_non_null_with_filter(
        ...
        for i in start..end {
            ...
            if filter_res {
                result_bridge
                    .append_non_null_int32_result(self.dictionary_page.find_int32(index))?;
            }
        }
    ...
    pub fn read_nullable_with_filter(
        ...
        for i in self.nullable_index..end {
            if validity[i] {
                ...
                data_with_nulls[i] = Some(self.dictionary_page.find_int32(index));
...
mod tests {
    ...
    fn load_dictionary_page(buf: &mut dyn ByteBufferBase) -> Rc<DictionaryPageEnum> {
        ...
        let dictionary_page = DictionaryPageInt32::new(&page_header, buf, mem::size_of::<i32>());
        assert!(dictionary_page.is_ok());
        Rc::from(DictionaryPageEnum::DictionaryPageInt32(
    ...
    fn load_dictionary_page_with_filter(
        ...
        let page_header = page_header.unwrap();
        let dictionary_page =
            DictionaryPageWithFilterInt32::new(&page_header, buf, mem::size_of::<i32>(), filter);
        assert!(dictionary_page.is_ok());
        Rc::from(DictionaryPageEnum::DictionaryPageWithFilterInt32(
            dictionary_page.unwrap(),
        ))
    }
    ...
    fn load_rle_bp_page<'a>(
        ...
        let data_page = RleBpDataPageReaderInt32V1::new(
            &page_header,
            buf,
            offset,
            4,
    ...
    fn test_create_rle_bp_data_page_v1() {
        let path = String::from("src/sample_files/rle_bp_integer_column.parquet");
    ...
    fn test_create_nullable_rle_bp_data_page_v1() {
        let path = String::from("src/sample_files/rle_bp_integer_column_with_nulls.parquet");
        let res = LocalFileLoader::new(&path);
        assert!(res.is_ok());
        let file = res.unwrap();
        let res = DirectByteBuffer::from_file(&file, 4, file.get_file_size() - 4);
    ...
    fn test_read_non_null_data_page() {
        ...
        while begin < num_values {
            ...
            assert!(res.is_ok());
            verify_rle_bp_int32_non_null_result(&result_row_range_set, &raw_bridge, None);
            begin = end;
        }
    }
    ...
    fn test_read_nullable_data_page() {
        ...
        while begin < num_values {
            ...
            let res = data_page.read(to_read, offset, &mut result_row_range_set, &mut raw_bridge);
            assert!(res.is_ok());
            verify_rle_bp_int32_nullable_result(&result_row_range_set, &raw_bridge, None);
            begin = end;
        }
    }
    ...
    fn test_read_non_null_data_page_with_filter() {
        ...
        while begin < num_values {
            ...
            assert!(res.is_ok());
            verify_rle_bp_int32_non_null_result(&result_row_range_set, &raw_bridge, Some(&filter));
            begin = end;
        }
    }
