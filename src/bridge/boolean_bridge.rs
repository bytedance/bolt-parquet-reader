...
pub struct BooleanBridge {
    ...
    non_null_data: Vec<bool>,
    nullable_data: Vec<Option<bool>>,
}
#[allow(dead_code)]
impl std::fmt::Display for BooleanBridge {
...
impl BooleanBridge {
    pub fn new(may_has_null: bool, capacity: usize) -> BooleanBridge {
        BooleanBridge {
            bridge_type: BridgeType::RustVec,
            bridge_data_type: BridgeDataType::Boolean,
            may_has_null,
...
impl ResultBridge for BooleanBridge {
    fn get_bridge_name(&self) -> String {
        String::from("Boolean Bridge")
    }
    ...
    fn append_non_null_bool_result(&mut self, result: bool) -> Result<(), BoltReaderError> {
        if unlikely(self.may_has_null) {
            return self.append_nullable_bool_result(Some(result));
        }
    ...
    fn append_non_null_bool_results(&mut self, result: &[bool]) -> Result<(), BoltReaderError> {
        if unlikely(self.may_has_null) {
            let nullable_results: Vec<Option<bool>> = result.iter().map(|&x| Some(x)).collect();
            return self.append_nullable_bool_results(&nullable_results);
        }
        let old_length = self.non_null_data.len();
        let new_length = old_length + result.len();
        self.non_null_data.resize(new_length, bool::default());
    ...
    fn to_bool_arrow_array(&mut self) -> Result<BooleanArray, BoltReaderError> {
        if self.may_has_null {
            Ok(BooleanArray::from(mem::take(&mut self.nullable_data)))
        } else {
            Ok(BooleanArray::from(mem::take(&mut self.non_null_data)))
        }
    }
            ...
            while self_range_idx < self_ranges.get_row_ranges().len() {
                ...
                if other_range_begin >= self_range_begin && other_range_end <= self_range_end {
                    ...
                    if self.may_has_null() {
                        ...
                        result_bridge.append_nullable_bool_results(value_slice)?;
                    } else {
                        let value_slice = &self.non_null_data[self_start..self_start + other_len];
                        result_bridge.append_non_null_bool_results(value_slice)?;
                    }
...
mod tests {
    ...
    fn get_non_null_boolean_value(index: usize) -> bool {
        index % 4 == 0
    }

    fn get_nullable_boolean_value(index: usize) -> Option<bool> {
    ...
    fn create_non_null_boolean_data(row_range_set: &RowRangeSet) -> Vec<bool> {
        ...
        for row_range in row_range_set.get_row_ranges() {
            for i in row_range.begin..row_range.end {
                bool_vec.push(get_non_null_boolean_value(i + offset));
            }
        }
    ...
    fn create_nullable_boolean_data(row_range_set: &RowRangeSet) -> Vec<Option<bool>> {
        let mut bool_vec = Vec::new();

        let offset = row_range_set.get_offset();
        for row_range in row_range_set.get_row_ranges() {
            for i in row_range.begin..row_range.end {
                bool_vec.push(get_nullable_boolean_value(i + offset));
            }
        }
    ...
    fn create_boolean_raw_bridge(
        may_has_null: bool,
        capacity: usize,
        row_range_set: &RowRangeSet,
    ) -> BooleanBridge {
        let mut bool_raw_bridge: BooleanBridge = BooleanBridge::new(may_has_null, capacity);

        let offset = row_range_set.get_offset();
        for row_range in row_range_set.get_row_ranges() {
            for i in row_range.begin..row_range.end {
                if may_has_null {
                    let _ = bool_raw_bridge
                        .append_nullable_bool_result(get_nullable_boolean_value(i + offset));
                } else {
                    let _ = bool_raw_bridge
                        .append_non_null_bool_result(get_non_null_boolean_value(i + offset));
                }
            }
        }
    ...
    fn verify_boolean_raw_bridge_data(raw_bridge: &BooleanBridge, row_range_set: &RowRangeSet) {
        let offset = row_range_set.get_offset();
        for row_range in row_range_set.get_row_ranges() {
            for i in row_range.begin..row_range.end {
                let (validity, value) = raw_bridge
                    .get_bool_validity_and_value(offset, i, &row_range_set)
                    .unwrap();
    ...
    fn test_create_raw_bridge() {
        ...
        let raw_bridge: BooleanBridge = BooleanBridge::new(may_has_null, capacity);
        ...
        assert_eq!(raw_bridge.get_bridge_name(), "Boolean Bridge");
    }
    ...
    fn test_append_result() {
        ...
        let raw_bridge = create_boolean_raw_bridge(may_has_null, capacity, &row_range_set);

        assert_eq!(raw_bridge.get_size(), raw_bridge_size);

        let first_element = raw_bridge.get_bool_validity_and_value(3, 1, &row_range_set);
        assert!(first_element.is_ok());
        assert_eq!(first_element.unwrap(), (true, true));

        let first_element = raw_bridge.get_bool_validity_and_value(2, 2, &row_range_set);
        assert!(first_element.is_ok());
        assert_eq!(first_element.unwrap(), (true, true));

        verify_boolean_raw_bridge_data(&raw_bridge, &row_range_set);

        let non_existing_res = raw_bridge.get_bool_validity_and_value(offset, 11, &row_range_set);
    ...
    fn test_append_results() {
        ...
        let values_to_append = create_non_null_boolean_data(&row_range_set);
        let mut raw_bridge = BooleanBridge::new(may_has_null, capacity);

        assert!(raw_bridge
            .append_non_null_bool_results(&values_to_append)
            .is_ok());
    ...
    fn test_append_nullable_results() {
        ...
        let values_to_append = create_nullable_boolean_data(&row_range_set);
        let mut raw_bridge = BooleanBridge::new(may_has_null, capacity);

        assert!(raw_bridge
            .append_nullable_bool_results(&values_to_append)
            .is_ok());
    ...
    fn test_convert_to_nullable_bridge() {
        ...
        let mut raw_bridge = create_boolean_raw_bridge(may_has_null, capacity, &row_range_set);
        assert_eq!(raw_bridge.may_has_null(), false);
    ...
    fn test_transfer_random_non_null_results() {
        ...
        for _ in 0..num_tests {
            ...
            let mut self_raw_bridge =
                create_boolean_raw_bridge(may_has_null, capacity, &self_range);
            let mut other_raw_bridge = BooleanBridge::new(may_has_null, capacity);
            assert!(self_raw_bridge
                .transfer_values(&self_range, &other_range, &mut other_raw_bridge)
                .is_ok());

            verify_boolean_raw_bridge_data(&other_raw_bridge, &other_range);
        }
    }
    ...
    fn test_convert_to_arrow() {
        ...
        assert!(raw_bridge.to_bool_arrow_array().is_ok());
    }
}
